function parse_OBJ(text) {
    ...
    function process_OBJ(text) {
        ...
        var norms = [];     // maybe recalc?
        var verts = [];
        var uvs = [];
        var tag_to_index = new Map;

        var elems = [];
        var n_elems = data.pos_index.length;
        for (var i = 0; i < n_elems; ++i) {
            var pos_index = data.pos_index[i];
            var nor_index = data.nor_index[i];
            var tex_index = data.tex_index[i];

            var tag = ((pos_index * n_elems) + tex_index) * n_elems + nor_index;
        ...
        return {
            ...
            elements: new Uint32Array(elems),
